  ...
  for (const p of candidates) {
    try {
      if (fs.existsSync(path.join(p, "index.html"))) {
        console.log("Client dist found:", p);
        return p;
      }
    } catch {
    }
...
var clientDist = resolveClientDist();
...
var io = new Server(server, { cors: { origin: "*", methods: ["GET", "POST"] }, maxHttpBufferSize: 1e8 });
app.use(cors());
app.use(express.json({ limit: "50mb" }));
app.use(express.static(clientDist));
var users = /* @__PURE__ */ new Map();
var usersByName = /* @__PURE__ */ new Map();
var sessions = /* @__PURE__ */ new Map();
var friendships = /* @__PURE__ */ new Map();
var friendRequests = [];
...
function ensureFriendSet(uid) {
  if (!friendships.has(uid)) friendships.set(uid, /* @__PURE__ */ new Set());
}
app.post("/api/register", async (req, res) => {
  ...
  if (usersByName.has(clean.toLowerCase())) return res.status(400).json({ error: "username taken" });
  const id = uuidv4();
  const hash = await bcrypt.hash(password, 8);
  const user = { id, username: clean, passwordHash: hash, avatar: avatar || `https://api.dicebear.com/7.x/initials/svg?seed=${encodeURIComponent(clean)}`, bio: bio || "\u041F\u0440\u0438\u0432\u0435\u0442! \u042F \u0438\u0441\u043F\u043E\u043B\u044C\u0437\u0443\u044E Cbopka", status: "online", createdAt: Date.now(), lastSeen: Date.now() };
  users.set(id, user);
  usersByName.set(clean.toLowerCase(), id);
  ensureFriendSet(id);
  const token = uuidv4();
...
app.post("/api/login", async (req, res) => {
  ...
  const uid = usersByName.get(username.trim().toLowerCase());
...
app.get("/api/users/search", (req, res) => {
  ...
  const list = [...users.values()].filter((u) => u.username.toLowerCase().includes(q) && u.id !== uid).slice(0, 20).map(getUserPublic);
  res.json({ users: list });
});
app.get("/api/friends", (req, res) => {
  ...
  if (!uid) return res.status(401).json({ error: "unauthorized" });
  ensureFriendSet(uid);
  const friends = [...friendships.get(uid) || []].map((fid) => getUserPublic(users.get(fid))).filter(Boolean);
  const incoming = friendRequests.filter((r) => r.to === uid && r.status === "pending").map((r) => ({ ...r, fromUser: getUserPublic(users.get(r.from)) }));
  const outgoing = friendRequests.filter((r) => r.from === uid && r.status === "pending").map((r) => ({ ...r, toUser: getUserPublic(users.get(r.to)) }));
...
app.get("/api/groups", (req, res) => {
  ...
  const uid = sessions.get(token);
  if (!uid) return res.status(401).json({ error: "unauthorized" });
  const myGroups = [...groups.values()].filter((g) => g.members.has(uid)).map((g) => ({ ...g, members: [...g.members].map((mid) => getUserPublic(users.get(mid))), admins: [...g.admins] }));
...
app.get("/api/messages/:convoId", (req, res) => {
  ...
  const msgs = messages.get(convoId) || [];
...
io.on("connection", (socket) => {
  ...
  socket.on("user:update", ({ avatar, bio, status, username }) => {
    ...
    if (username && username.trim().length >= 3) {
      ...
      if (oldKey !== newKey && !usersByName.has(newKey)) {
        usersByName.delete(oldKey);
        usersByName.set(newKey, uid);
        user.username = username.trim();
      }
    }
    io.emit("user:updated", { user: getUserPublic(user) });
  });
  socket.on("friends:request", ({ toUsername, toUserId }) => {
    let targetId = toUserId;
    if (!targetId && toUsername) targetId = usersByName.get(toUsername.trim().toLowerCase());
    if (!targetId || targetId === uid) return;
    ensureFriendSet(uid);
    ensureFriendSet(targetId);
    if (friendships.get(uid)?.has(targetId)) return socket.emit("error_msg", { msg: "\u0423\u0436\u0435 \u0432 \u0434\u0440\u0443\u0437\u044C\u044F\u0445" });
    if (friendRequests.find((r) => r.from === uid && r.to === targetId && r.status === "pending")) return;
  ...
  socket.on("friends:accept", ({ requestId }) => {
    const fr = friendRequests.find((r) => r.id === requestId && r.to === uid);
    if (!fr) return;
    fr.status = "accepted";
    ensureFriendSet(fr.from);
    ensureFriendSet(fr.to);
    friendships.get(fr.from).add(fr.to);
    friendships.get(fr.to).add(fr.from);
    const fromUser = users.get(fr.from);
    const toUser = users.get(fr.to);
    ...
    io.to(s1).emit("friends:request:accepted", fr);
    io.to(s2).emit("friends:request:accepted", fr);
  });
  ...
  socket.on("friends:remove", ({ friendId }) => {
    ensureFriendSet(uid);
    ensureFriendSet(friendId);
    friendships.get(uid)?.delete(friendId);
    friendships.get(friendId)?.delete(uid);
    socket.emit("friends:removed", { friendId });
    const s = userSockets.get(friendId);
    if (s) io.to(s).emit("friends:removed", { friendId: uid });
  });
  socket.on("message:send", ({ convoId, text, toUserId, type = "text", meta }) => {
    ...
    if (finalConvo.includes("_")) {
      ...
      const g = groups.get(finalConvo);
      if (!g || !g.members.has(uid)) return;
    }
    const msg = { id: uuidv4(), from: uid, convoId: finalConvo, text: text.trim().slice(0, 2e3), at: Date.now(), type, meta: meta || null };
    if (!messages.has(finalConvo)) messages.set(finalConvo, []);
    messages.get(finalConvo).push(msg);
    if (finalConvo.includes("_")) {
      const parts = finalConvo.split("_");
      parts.forEach((pid) => {
  ...
  socket.on("group:create", ({ name, memberIds, avatar, description }) => {
    ...
    const id = uuidv4();
    const members = /* @__PURE__ */ new Set([uid, ...(memberIds || []).filter((mid) => users.has(mid))]);
    const g = { id, name: name.trim().slice(0, 40), avatar: avatar || `https://api.dicebear.com/7.x/shapes/svg?seed=${id}`, description: (description || "").slice(0, 200), members, admins: /* @__PURE__ */ new Set([uid]), createdAt: Date.now() };
    groups.set(id, g);
    members.forEach((pid) => {
      const sid = userSockets.get(pid);
      if (sid) io.to(sid).emit("group:created", { ...g, members: [...g.members].map((mid) => getUserPublic(users.get(mid))), admins: [...g.admins] });
    });
  });
  socket.on("group:addMembers", ({ groupId, memberIds }) => {
    ...
    memberIds.forEach((mid) => {
      if (users.has(mid)) g.members.add(mid);
    });
    g.members.forEach((pid) => {
      const sid = userSockets.get(pid);
      if (sid) io.to(sid).emit("group:updated", { ...g, members: [...g.members].map((mid) => getUserPublic(users.get(mid))), admins: [...g.admins] });
    });
  });
  ...
  socket.on("group:update", ({ groupId, name, avatar, description }) => {
    ...
    if (description !== void 0) g.description = description.slice(0, 200);
    g.members.forEach((pid) => {
      const sid = userSockets.get(pid);
      if (sid) io.to(sid).emit("group:updated", { ...g, members: [...g.members].map((mid) => getUserPublic(users.get(mid))), admins: [...g.admins] });
    });
  ...
  socket.on("call:invite", ({ toUserId, type = "video", groupId }) => {
    ...
    if (groupId) {
      const g = groups.get(groupId);
      if (!g || !g.members.has(uid)) return;
      const call = { id: callId, initiator: uid, type, groupId, participants: /* @__PURE__ */ new Set([uid]), startedAt: Date.now() };
      ...
      const call = { id: callId, initiator: uid, type, participants: /* @__PURE__ */ new Set([uid]), startedAt: Date.now(), toUserId };
      calls.set(callId, call);
  ...
  socket.on("disconnect", () => {
    ...
    if (u) {
      u.status = "offline";
      u.lastSeen = Date.now();
      socket.broadcast.emit("user:offline", { userId: uid, lastSeen: u.lastSeen });
    }
...
app.get("*", (req, res) => {
  res.sendFile(path.join(clientDist, "index.html"), (err) => {
    if (err) res.send("Cbopka server running. Build client first.");
  });
});
